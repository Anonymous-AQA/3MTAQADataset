...
class VideoDataset(Dataset):
    ...
    def __init__(self, mode, args):
        ...
        # loading annotations
        self.args = args
        self.annotations = pkl.load(open(os.path.join(info_dir, 'augmented_final_annotations_dict_text.pkl'), 'rb'))
    ...
    def proc_label(self, data):
        ...
        tmp = [stats.norm.pdf(np.arange(output_dim['MUSDL+LVFL']), loc=judge_score * (output_dim['MUSDL+LVFL']-1) / judge_max, scale=self.args.std).astype(np.float32)
